  ...
  return (
    <div>
      ...
      {currentReviews.map((review) => (
          <Reviews key={review.id} review={review} showUserAvatar={true} isFullWidth={true} />
          ))}
          </div>
            ...
            {Array(Math.ceil(allReviews.length / reviewsPerPage))
              ...
              .map((_, index) => (
                <li key={index} onClick={() => paginate(index + 1)}>
                  <button className={currentPage=== (index+1) ?  "btn btn-primary currentPage" : "btn btn-primary"}>{index + 1}</button>
                </li>
              ))}
          </ul>
        )}
