...
const useFetchByIdAndIngredient = (id) => {
  ...
  useEffect(() => {
    (async (id) => {
      ...
      if (cocktailResult && cocktailResult.data && cocktailResult.data.drinks && cocktailResult.data.drinks[0]) {
        setData(cocktailResult.data.drinks[0])

        const ingredient = cocktailResult.data.drinks[0].strIngredient1;

        const ingredientsResult = await axios.get(`${searchByIngredient}${ingredient}`);
        // console.log(ingredientsResult.data.drinks)
        if (ingredientsResult) {
          //seleziono 6 drink a caso
          const firstFiveIngredients = ingredientsResult.data.drinks.sort(() => 0.5 - Math.random()).slice(0, 6);
